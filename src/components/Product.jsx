...
const ProductCard = ({ data, onPreview, onClick, onDelete, onEdit }) => {
  ...
  return (
    <div
      className={`product ${onClick ? "category" : ""}`}
      onClick={() => onClick && onClick(data)}
    >
      <div className="product-details">
        <div className="product-image">
          {getMedia({
            image: categoryImage || (images && images[0]),
            index: 1,
            onPreview,
            data,
            autoPlay: false
          ...
          {shippingCharges &&
            !authState.hasPermissions([Permissions.SHOPKEEPER]) && (
              <p className="product-price shipping-charges">
                {Number(shippingCharges) ? shippingCharges : "Free Shipping"}{" "}
                (Shipping Charges)
              </p>
            )}
          {localShippingCharges &&
